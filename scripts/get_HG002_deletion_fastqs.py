...
def get_fastq(sample, args):
    n = sample.split("Sample")[1]
    return args.input_folder+"/HG0002."+n+".fastq.gz"

def get_output_fastq(sample, args):
    n = sample.split("Sample")[1]
    return args.output_fasta_folder+"/HG0002."+n+".fastq"
...
parser.add_argument('--counts', required=True, type=int)
parser.add_argument('--mat_prefix', default="reads_assembly_subset_mapped/HG0002.mat.sorted.")
parser.add_argument('--pat_prefix', default="reads_assembly_subset_mapped/HG0002.pat.sorted.")
parser.add_argument('--reads-to-ignore', required=False, default=None)
...
with open(args.centromeres) as in_cf:
    ...
    for line in in_cf:
        ...
        chrom = line_args[1]
        start = int(line_args[2]) - 100000
        end = int(line_args[3]) + 100000
...
with open(args.mat_ref_inserts, 'r') as in_tsv:
    for line in in_tsv:
        ...
        if len(nearby) > 0:
            continue
        if row[8] != "PASS" and row[8] != "multiply_mapped":
            continue
...
for i in range(args.counts):
    ...
    mat_bam = args.sample + "/" + args.mat_prefix + str(i+1) + ".bam"
    pat_bam = args.sample + "/" + args.pat_prefix + str(i+1) + ".bam"
    reads_with_inserts_per_sample[sample], depth_per_sample[sample] = get_inserts_for_sample(mat_bam, pat_bam, combined_mat_pat_ref_inserts, args.chr_list.split(","), args.reads_to_ignore)
...
for sample in reads_with_inserts_per_sample:
    for pos in reads_with_inserts_per_sample[sample]:
        ...
        if random.uniform(0, 1) < 0.3:
            ...
            # Sample1 is the control, select a sample out of the rest of the counts
            n = random.randint(2,args.counts)
            s = "Sample"+str(n)
            novel_positions[pos] = s
