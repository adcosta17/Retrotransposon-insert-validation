...
parser.add_argument('--annotation-folder', default="fastq_inserted")
            ...
            for line in in_data:
                ...
                if len(insert_list) == 0:
                    if row[3] in truth_set:
                        ...
                        insert_size = truth_set[row[3]].split('_')[0]
                        repeat_type = truth_set[row[3]].split('_')[1]
                        if row[8] == "PASS":
                            # Check more here
                ...
                if len(nearby) > 0:
                    if row[8] == "PASS":
                        ...
                        for item in nearby:
                            ...
                            tp_position[row[0]+":"+str(item.begin)+"-"+str(item.end)] = insert_size+"\t"+repeat_type+"\t"+str(len(reads_to_pos[pos]))
                    else:
                        for item in nearby:
                            pos = item.data.split('_')[0]
                            insert_size = item.data.split('_')[1]
                            repeat_type = item.data.split('_')[2]
                            fn_position[row[0]+":"+str(item.begin)+"-"+str(item.end)] = insert_size+"\t"+repeat_type+"\t"+str(len(reads_to_pos[pos]))
                else:
                    # Read we don't care about
                    if row[8] == "PASS":
                        insert_size = str(int(row[5])-int(row[4]))
                        ...
                        if "LINE" in line:
                            ...
                            repeat_type = "SVA"
                        fp_position[row[0]+":"+row[1]+"-"+row[2]] += insert_size+"\t"+repeat_type
                    else:
                        tn_position += 1
