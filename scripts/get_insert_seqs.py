...
def get_contig_insertion_seqs(hap_positions, hap_contigs, chromosome_lengths, centromeres, seqs):
    ...
    if chrom not in chromosome_lengths:
        return ""
    pos = random.randint(250000,chromosome_lengths[chrom]-250001)
    ...
    if found:
        ...
        left_flank = 250000
        right_flank = 250000
        if contig_pos < 250000:
            left_flank = contig_pos
        if len(hap_contigs[name]) - contig_pos < 250000:
...
for record in mat.fetch():
    if record.mapping_quality > 0 and len(mat_contigs[record.query_name]) > 500000:
        mapped_count[record.query_name][record.query_alignment_start:record.query_alignment_end] = 1
for record in pat.fetch():
    if record.mapping_quality > 0 and len(pat_contigs[record.query_name]) > 500000:
        mapped_count[record.query_name][record.query_alignment_start:record.query_alignment_end] = 1

mat_positions = defaultdict(IntervalTree)
pat_positions = defaultdict(IntervalTree)
# Read in contig to ref bam, store contigs that map with qual == 60, and positions that are not centromere alignments
...
with open(args.mat_out_fa, 'w') as out_fa:
    for record in mat.fetch():
        ...
        if len(nearby) > 1:
            continue
        if record.mapping_quality == 60 and record.reference_length > 500000:
