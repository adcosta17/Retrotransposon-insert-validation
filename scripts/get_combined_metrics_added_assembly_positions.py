...
parser.add_argument('--annotation-suffix', default=".insert_annotation.tsv")
parser.add_argument('--annotation-folder', default="fastq_insert_added_position")
...
for genome_id in args.id_list.split(','):
    ...
    with open(genome_id+"/"+args.annotation_folder+"/"+genome_id+args.annotation_suffix, 'r') as in_an:
        with pysam.FastaFile(genome_id+"/fastq_insert_added_position/"+genome_id+"_3.fastq.gz") as fin_1:
             with pysam.FastaFile(genome_id+"/fastq_insert_added_position/"+genome_id+"_5.fastq.gz") as fin_2:
                for line in in_an:
                    if count == 0:
                        count = 1
                        continue
                    ...
                    if "3" in row[0]:
                        sample = "Sample3"
                        read_seq = fin_1.fetch(row[1])
                    else:
                        sample = "Sample5"
                        read_seq = fin_2.fetch(row[1])
                    if int(row[2]) - int(row[3]) < 500:
                        cut_start += 1
                    if len(read_seq) - int(row[2]) - int(row[4]) < 500:
                        cut_end += 1
                    truth_set[sample][row[1]] = row[:-1]
    #print(cut_start)
    #print(cut_end)
        ...
        with open(args.input_prefix+"/"+genome_id+"/"+sample+"/"+args.folder+"/"+sample+args.suffix, 'r') as in_data:
            ...
            for line in in_data:
                ...
                if row[3] in truth_set[sample]:
                    ...
                    if row[8] == "PASS":
                        # Check more here
                        tp[row[3]] = insert_size+"\t"+repeat_type+"\t"+str(read_depth[row[3]])
                    else:
                    ...
                    if row[8] == "PASS":
                        ...
                        if "LINE" in line:
                            ...
                            repeat_type = "SVA"
                        fp[row[3]] = insert_size+"\t"+repeat_type
                        #reads_to_check[row[3]] = 1
                    else:
                        tn += 1
                #else:
...
print("Miseed FN: \t"+str(missed_fn_overall))
print("Insert Size: ")
