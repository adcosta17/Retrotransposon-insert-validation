...
def select_reads_to_delete(insert_positions, delete_all, deletion_fraction, depth_per_pos, novel_positions, sample):
...
depth_per_sample = {}
reads_with_inserts_per_sample["Control"], depth_per_sample["Control"] = get_inserts_for_sample(args.mat_bam_control, args.pat_bam_control, combined_mat_pat_ref_inserts, args.chr_list.split(","),args.reads_to_ignore, True)
reads_with_inserts_per_sample["Test"], depth_per_sample["Test"] = get_inserts_for_sample(args.mat_bam_test, args.pat_bam_test, combined_mat_pat_ref_inserts, args.chr_list.split(","),None, False)
...
for sample in ["Control", "Test"]:
    ...
    if sample == "Control":
        delete_all = True
    deletion_positions, retained_positions, count_and_depth_per_pos = select_reads_to_delete(reads_with_inserts_per_sample[sample], delete_all, deletion_fraction, depth_per_sample[sample], novel_positions, sample)
    with open(get_output_fastq(sample, args), 'w') as fout:
            ...
            for entry in fin:
                ...
                if entry.name in deletion_positions and entry.name in retained_positions:
                    ...
                    for i in range(len(deletion_positions_list)):
                        if rp_count < len(retained_positions_list):
                            if deletion_positions_list[i][0] > retained_positions_list[rp_count][0]:
                                ...
                                print(sample+"\t"+entry.name+"\t"+str(retained_positions_list[rp_count][0]-deleted_count)+"\t"+str(retained_positions_list[rp_count][1]-deleted_count)+"\t"+str(retained_positions_list[rp_count][2])+"\t"+count_and_depth_per_pos[str(retained_positions_list[rp_count][2])]+"\t"+retained_positions_list[rp_count][3])
                                rp_count+= 1
                        # Delete the seqeunce
                    ...
                    for i in range(len(retained_positions_list)):
                        if i < rp_count:
                            continue
                        print(sample+"\t"+entry.name+"\t"+str(retained_positions_list[i][0]-deleted_count)+"\t"+str(retained_positions_list[i][1]-deleted_count)+"\t"+str(retained_positions_list[i][2])+"\t"+count_and_depth_per_pos[str(retained_positions_list[i][2])]+"\t"+retained_positions_list[i][3])
                    ...
                    retained_positions_list.sort()
                    for i in range(len(retained_positions_list)):
                        print(sample+"\t"+entry.name+"\t"+str(retained_positions_list[i][0])+"\t"+str(retained_positions_list[i][1])+"\t"+str(retained_positions_list[i][2])+"\t"+count_and_depth_per_pos[str(retained_positions_list[i][2])]+"\t"+retained_positions_list[i][3])
                if len(seq) < 1:
                    continue
